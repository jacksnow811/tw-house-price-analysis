...
| `.claude/skills/price-negotiation/` | Claude Code skill：給大樓物件 → 讀 `data/` 社區實價登錄算實坪制坪價行情、讀 `remark/` 社區景觀備註分**對外/對內**算溢價 → 即時查行情 → 站在**買方**產出三段式報價＋談判話術（`/price-negotiation`）。含 `scripts/comps_stats.py`、`reference/談價方法論.md` |
| `data/*.csv` | 抓取結果（gitignore，依條件自動命名） |
| `remark/` | 人工維護的社區景觀備註（對內／對外，依地址比對），供 price-negotiation 分組算溢價；格式見 `remark/README.md`（建議 commit） |
