# housing-analysis Specification

## Purpose

提供一個 Claude Code skill（`/housing-analysis`），讓使用者僅以一個社區/建案名稱即可評估「這個建案值不值得買」：先確認建案與其建商，再上網蒐集評價與新聞——務必嘗試台灣常用社群（PTT、Dcard、Threads、Facebook）——最後依固定模板產出建案總覽、建商分析（必備子環節）、建案優點/缺點、優劣總評與參考的歷史/類似建案案例，並標註來源、區分事實與推論。本能力取代原 `builder-analysis`，對應 `.claude/skills/housing-analysis/`，依 Anthropic 官方 Agent Skills 標準建立。
